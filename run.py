...
def index():
    ...
    pj_jurusan = len(jurusan)
    pj_mahasiswa = len(mahasiswa)
    pj_dosen = len(dosen)
    pj_mk = len(mk)
    pj_nilai = len(nilai)
    print(pj_jurusan)
    context = {
        'pj_jurusan': pj_jurusan,
        'pj_mahasiswa': pj_mahasiswa,
        'pj_dosen':pj_dosen,
        'pj_mk':pj_mk,
        'pj_nilai':pj_nilai,
...
def addmh():
    if request.method == 'POST':
        ...
        cursor = db.cursor()
        query = f"INSERT INTO tb_mahasiswa VALUES ('{nim}','{nama}','{jenis_kelamin}','{tempat_lahir}','{tanggal_lahir}','{alamat}','{kode_prodi}')"
        cursor.execute(query)
        db.commit()
